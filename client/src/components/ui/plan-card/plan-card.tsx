	...
	return (
		<article
			className={`rounded-3xl border p-6 shadow-[0_14px_40px_rgba(34,61,102,0.08)] ${
				highlight ? "border-[#cfe0fc] bg-[#f8fbff]" : "border-[#e5eaf4] bg-white"
			}`}
		>
			<p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#5f6368]">{title}</p>
			<p className="mt-3 text-4xl font-semibold text-[#202124]">{price}</p>
			<p className="mt-2 text-sm leading-6 text-[#5f6368]">{description}</p>

			<ul className="mt-5 space-y-2 text-sm text-[#202124]">
			...
			<button
				className={`mt-6 w-full rounded-full border px-4 py-2 text-sm font-semibold transition ${
					highlight
						? "border-[#1a73e8] bg-[#1a73e8] text-white hover:bg-[#175fc0]"
						: "border-[#d9e1ef] bg-white text-[#202124] hover:bg-[#f8f9fa]"
				}`}
			>
				{actionLabel}
			</button>
